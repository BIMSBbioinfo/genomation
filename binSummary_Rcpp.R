...
myFunc_Mean<-function(bwFile,promoter.gr){
...
myFunc_Median<-function(bwFile,promoter.gr){
  cov.bw=import(bwFile, which=promoter.gr,as = "RleList");
  mcols(promoter.gr)$X_rank = 1:length(promoter.gr)
  ...
  mat_res <- listSliceMedian(do.call("c",mat),10);
  ## copied from scoreMatrix.R
...
myFunc_Max<-function(bwFile,promoter.gr){
  mcols(promoter.gr)$X_rank = 1:length(promoter.gr);
  cov.bw=import(bwFile, which=promoter.gr,as = "RleList");
  myViews=Views(cov.bw,as(promoter.gr,"RangesList")); # get subsets of coverage
  ...
  mat_res = listSliceMax(do.call("c",mat), 10);
  ## copied from scoreMatrix.R
  # get the ranks of windows, when things are reorganized by as(...,"RangesList")
  r.list=split(mcols(promoter.gr)[,"X_rank"], as.vector(seqnames(promoter.gr))  )
  r.list=r.list[order(names(r.list))]
...
myFunc_Min<-function(bwFile,promoter.gr){
  mcols(promoter.gr)$X_rank = 1:length(promoter.gr);
  cov.bw=import(bwFile, which=promoter.gr,as = "RleList");
  myViews=Views(cov.bw,as(promoter.gr,"RangesList")); # get subsets of coverage
  ...
  mat_res = listSliceMin(do.call("c",mat), 10);
  ## copied from scoreMatrix.R
  # get the ranks of windows, when things are reorganized by as(...,"RangesList")
  r.list=split(mcols(promoter.gr)[,"X_rank"], as.vector(seqnames(promoter.gr))  )
  r.list=r.list[order(names(r.list))]
...
microbenchmark(
  myFunc_Mean(bwFile,bigp),
  ScoreMatrixBin(bwFile, bigp,bin.num = 10,bin.op="mean",type="bigWig"),
  myFunc_Median(bwFile,bigp),
  myFunc_Max(bwFile,bigp),
  myFunc_Min(bwFile,bigp)
)
