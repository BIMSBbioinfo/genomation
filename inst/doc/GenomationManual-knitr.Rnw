...
A standard step of a computational genomics experiment is the visualization of 
average genomic profiles over a certain predefined set of range, such as a visualization
of enrichment of a certain histone modification around a transcription factor binding site,
...
\Rcode{scoreMatrixBin} designed for use with windows of unequal width (e.g. methylation
over cpg islands)
...
As an example we will extrat the density of cage tags around the promoters in 
the human genome
<<extractCAGE, eval=FALSE, tidy=TRUE>>=
data(cage)
data(promoters)
