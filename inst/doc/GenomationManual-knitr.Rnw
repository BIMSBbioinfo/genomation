...
\code{narrowPeak} and \code{broadPeak} formats, and gtf formatted files.
...
to the ranges that were used to construct the windows (e.g. row name 10 means that the
10th element in the target GRanges object was used to extract the data). If a certain
rowname is not present in the ScoreMatrix object, that means that te corresponding
range was filtered out (e.g. the range could have been on a chromosome that was not
present in the target).
\\
To simultaneously work on multiple files you can use the \Rcode{ScoreMatrixList}
function. The function also has 2 obligatory arguments \code{targets} and \code{windows}.
While the \code{windows} is the same as in \Rcode{ScoreMatrix}, the targets argument
contains results from multiple experiments. 
It can be in one of the three formats: a list of RleLists, a list of GRanges (or a GRangesList object), 
or a character vector designating a set of .bam or .bigWig files.
...
or as a histogram. \Rcode{heatMatrix]}, \Rcode{plotMeta} and \Rcode{multiHeatMatrix} are functions for visualization of \Rcode{ScoreMatrix} and \Rcode{ScoreMatrixList} objects.

We will plot the distribution of CAGE tags around promoters on human chr21.
<<heatMatrix1, eval=TRUE, tidy=TRUE, fig.width=6, fig.height=4, out.width='.49\\linewidth', fig.keep='last'>>=
data(cage)
data(promoters)
sm = ScoreMatrix(target=cage, windows=promoters)
...
<<heatMatrix2, eval=TRUE, tidy=TRUE, fig.width=6, fig.height=4, out.width='.49\\linewidth'>>=
data(cage)
data(promoters)
data(cpgi)
...
# cpg.ind = factor(countOverlaps(promoters, cpgi)>0, levels=c('CpG','noCpG'))
# heatMatrix(sm, xcoords=c(-1000, 1000), group=cpg.ind)
@
...
<<heatMatrixScales, eval=TRUE, tidy=TRUE, echo=TRUE, fig.width=6, fig.height=4, out.width='.49\\linewidth'>>=
...
<<multiHeatMatrix1, eval=TRUE, tidy=TRUE, fig.width=6, fig.height=4, out.width='.49\\linewidth'>>=
cage$tpm = NULL
targets = list(cage=cage, cpgi=cpgi)
...
<<multiHeatMatrix2, eval=TRUE, tidy=TRUE, fig.width=6, fig.height=4, out.width='.49\\linewidth'>>=
...
sampleInfo = read.table(file.path(genomationDataPath, 'SamplesInfo.txt'), header=T, sep='\t', stringsAsFactors=FALSE)
...
a bed12 formatted file and parses the coordinates into a \code{GRangesList} containing
...
CTCF peaks and gene structures. We will again use the \code{intersect.chr=TRUE}
...
percentage of overlap given by priority. If \code{cluster=TRUE}, then the function
will use hierarhical clustering to order the heatmap.
<<plotGeneAnnotation, eval=TRUE, tidy=TRUE, fig.width=6, fig.height=4, out.width='.49\\linewidth'>>=
...
<<ctcfScoreMatrixList, eval=TRUE, tidy=TRUE, fig.width=12, fig.height=8, out.width='.8\\linewidth'>>=
...
<<plotScaledProfile, eval=TRUE, tidy=TRUE, fig.width=12, fig.height=8, out.width='.8\\linewidth'>>=
...
<<visualizeFeatureComb, eval=TRUE, tidy=TRUE, fig.width=12, fig.height=8, out.width='.8\\linewidth'>>=
