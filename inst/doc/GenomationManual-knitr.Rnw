...
One of the big hindrances in computational genomics stems from the myriad of formats
that are used to store data. Although some formats have been selected as de facto standards
for certain kind of biological data (e.g. BAM, VCF), 
