...
#' @param chrom.sizes sizes of chromosomes as a named vector (names are chromsomes names and elements of the vectors are lengths). , if not given sizes in GRanges object will be used if no sizes there the end of each chr will be the end last feature on each chr
        ...
        function( feature,chrom.sizes ,stranded ,keep.strand.prop, keep.chrom,exclude,include,seed ){
          ...
          if(!is.null(chrom.sizes) ){
           ...
           max.inc=GRanges(seqnames=names(chrom.sizes),
                            ranges  =IRanges( start=rep(1,length(chrom.sizes)), width=chrom.sizes)
                            )            
          }
          ...
          if( (  !is.null(include) & is(include,"GRanges") )  | ( !is.null(exclude) & is(exclude,"GRanges")  )     ){
            ...
            if(!exists("max.inc")){
                ...
                #max.inc=GRanges(seqnames=chrs,ranges=IRanges( start=my.starts, end=my.ends))
                max.inc=GRanges(seqnames=chrs,ranges=IRanges( start=rep(1,length(my.ends)), end=my.ends))
            }
          ...
          if(keep.chrom)
          {
            if( ! exists("max.inc")) # if a maximal inclusion range NOT defined, find the range accordingly
            {
                  ...
                  #unlist(mapply(my.runif,Ns,ends)
                    ...
                    }else if(!stranded){ # if you  want to randomly assign strands
               ...
               return(randomized)
            }else{ # if a maximal inclusion range defined, find the range accordingly
              
                  
                  chrs1=as.character(unique(seqnames(max.inc)))
                  chrs2=as.character(unique(seqnames(feature)))
                  chrs=chrs1[chrs1 %in% chrs2] # get only chrs that are in both constraint and feature
                  ...
                  my.runif=function(n,x,y) round(runif(n, min=x, max=y)) 
                    ...
                    if(keep.strand.prop & stranded){
                          randomized=GRanges(seqnames=rep(names(Ns),Ns),
                                        ranges  =IRanges( start=unlist(mapply(my.runif,Ns,starts,ends)), width= width(feature1)),
