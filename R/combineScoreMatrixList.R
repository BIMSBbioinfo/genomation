...
#  when a ScoreMatrixListControl is a first argument
c.ScoreMatrixListControl<-function(..., recursive = FALSE, use.names = TRUE) {
  y <- list(...)
  n <- length(y)
  y3 <- list()
  Ln <- vector()
  ...
  for (i in 1:n){
    #combine the scoreMatrixListControl object
    if (is(y[[i]], "ScoreMatrixListControl")){
      y2 <- as(y[[i]], "list")
      y3 <- append(y3, y2)
      #combine its label name
      Ln <- append(Ln, names(y[[i]]))
      #combine the scoreMatrixControl object
    }else if(is(y[[i]], "ScoreMatrixControl")){
      y3 <- append(y3, list(y[[i]]))
      #combine its label name
      if (identical(c(""), names(y[i])) || is.null(names(y[i]))) {
        Ln <- append(Ln, c(""))
  ...
  #create the ScoreMatrixListControl
  SMLC <- as(y3, "ScoreMatrixListControl")
  names(SMLC) <- Ln
...
# ---------------------------------------------------------------------------- #
#  when a ScoreMatrixControl is a first argument

c.ScoreMatrixControl<-function(..., recursive = FALSE, use.names = TRUE) {
  y <- list(...)
  n <- length(y)
  y3 <- list()
  Ln <- vector()
