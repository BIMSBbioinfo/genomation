...
setMethod("getRandomEnrichment",signature(target = "GRanges",query="GRanges"),
        function( target,query,randomizations,rand.set,... ){
          ...
          }else{stop("Wrong 'rand.set' argument supplied, it should be a 'GRangesList'")}
          ...
          list(orig.cnt=orig.cnt,rand.olap.dist=rand.olap.dist,fc=log2(orig.cnt/mean(rand.olap.dist)),
               p.value=p.value,rand.p.value=rand.p.value)
