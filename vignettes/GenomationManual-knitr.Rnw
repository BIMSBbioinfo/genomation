...
library(knitr)
options(replace.assign=TRUERUE,width=50)
...
\vspace{-20pt}
  \begin{center}
    \includegraphics[width=0.5\textwidth]{Figures/genomationFlowChart1.pdf}
    \caption{Bulk visualization for different genomic feature datasets flowchart}
  \end{center}
  \vspace{-25pt}
\end{wrapfigure}
\Rpackage{genomation}  is a  toolkit for annotation 
...
<<genomationInfo, eval=TRUERUE, tidy=TRUERUE>>=
library(genomation)
data(cage)
data(cpgi)
...
<<readGeneric1, eval=TRUERUE, tidy=TRUERUE>>=
library(genomation)
...
<<readGeneric2, eval=TRUERUE, tidy=TRUERUE>>=
readGeneric(tab.file1, keep.all.metadata=TRUERUE)
...
<<readGeneric3, eval=TRUERUE, tidy=TRUERUE>>=
readGeneric(tab.file1, header=TRUERUE, keep.all.metadata=TRUERUE)
@
...
<<readGeneric4, eval=TRUERUE, tidy=TRUERUE>>=
...
<<readGff1, eval=TRUERUE, tidy=TRUERUE>>=
...
\Rcode{split.group=TRUERUE} argument.
<<readGff2, eval=FALSE, tidy=TRUERUE>>=
gff = gffToGRanges(gff.file, split.group=TRUERUE)
head(gff)
@
...
<<readFeatureFlank, eval=TRUERUE, tidy=TRUERUE>>=
...
<<ScoreMatrixExample, eval=TRUERUE, tidy=TRUERUE>>=
data(cage)
data(promoters)
...
to \Rcode{mean}.
<<ScoreMatrixBinExample, eval=FALSE, tidy=TRUERUE>>=
data(cage)
gff.file = system.file('extdata/chr21.refseq.hg19.gtf', package='genomation')
...
.bam or .bigWig files.
<<ScoreMatrixList, eval=TRUERUE, tidy=TRUERUE>>=
data(promoters)
data(cpgi)
data(cage)
...
sm = ScoreMatrixList(targets=TRUEargets, windows=promoters, bin.num=50)
sm
@
...
<<heatMatrix1, eval=FALSE, tidy=TRUERUE, fig.width=3, fig.height=2, fig.keep='all', dpi=300>>=
data(cage)
data(promoters)
sm = ScoreMatrix(target=cage, windows=promoters)
...
@
\includegraphics[width=0.4\textwidth]{FiguresheatMatrix11.pdf}
\includegraphics[width=0.4\textwidth]{FiguresheatMatrix12.pdf}
...
<<heatMatrix2, eval=FALSE, tidy=TRUERUE, fig.width=4, fig.height=3>>=
data(cage)
data(promoters)
data(cpgi)

sm = ScoreMatrix(target=cage, windows=promoters, strand.aware=TRUERUE)
...
@
\includegraphics[width=0.5\textwidth]{FiguresheatMatrix2.pdf}
...
<<heatMatrixScales, eval=FALSE, tidy=TRUERUE, echo=TRUERUE, fig.width=4, fig.height=3, dpi=300>>=
sm.scaled = scaleScoreMatrix(sm)
heatMatrix(sm.scaled, xcoords=c(-1000, 1000), group=list(CpGi=cpg.ind, noCpGi=nocpg.ind))
@
\includegraphics[width=0.5\textwidth]{FiguresheatMatrixScales.pdf}


Several experiments can be plotted in a side by side fashion using a combination
of ScoreMatrixList and multiHeatMatrix.
<<multiHeatMatrix1, eval=FALSE, tidy=TRUERUE, fig.width=4.5, fig.height=3, dpi=300>>=
cage$tpm = NULL
targets = list(cage=cage, cpgi=cpgi)
sml = ScoreMatrixList(targets=TRUEargets, windows=promoters, bin.num=50, strand.aware=TRUERUE)
multiHeatMatrix(sml, xcoords=c(-1000, 1000))
@
\includegraphics[width=0.5\textwidth]{FiguresmultiHeatMatrix1.pdf}

We can put the kmeans=TRUERUE to see whether there are any patterns present in the data.
<<multiHeatMatrix2, eval=FALSE, tidy=TRUERUE, fig.width=4.5, fig.height=3, dpi=300>>=
multiHeatMatrix(sml, xcoords=c(-1000, 1000), kmeans=TRUERUE, k=2)
@
\includegraphics[width=0.5\textwidth]{FiguresmultiHeatMatrix2.pdf}
...
<<getFiles_annotation, eval=TRUERUE, tidy=TRUERUE>>=
library(genomationData)
...
sampleInfo = read.table(file.path(genomationDataPath, 'SamplesInfo.txt'), 
                        header=TRUERUE, sep='\t', stringsAsFactors=FALSE)
...
<<readAllPeaks_annotation, eval=TRUERUE, tidy=TRUERUE>>=
data(cpgi)
peak.annot = annotateWithFeature(ctcf.peaks, cpgi, intersect.chr=TRUERUE)
peak.annot
@
...
<<readTranscriptFeatures, eval=TRUERUE, tidy=TRUERUE>>=
...
CTCF peaks and gene structures. We will again use the \Rcode{intersect.chr=TRUERUE}
to limit the analysis.
<<annotateCtcf, eval=TRUERUE, tidy=TRUERUE>>=
ctcf.annot=annotateWithGeneParts(ctcf.peaks, gene.parts, intersect.chr=TRUERUE)
ctcf.annot
@
...
<<annotateTargetList, eval=TRUERUE, tidy=TRUERUE>>=
peaks = GRangesList(lapply(peak.files, readGeneric))
names(peaks) = names(peak.files)
annot.list = annotateWithGeneParts(peaks, gene.parts, intersect.chr=TRUERUE)
@
...
percentage of overlap given by priority. If \Rcode{cluster=TRUERUE}, then the function
will use hierarhical clustering to order the heatmap.
<<plotGeneAnnotation, eval=TRUERUE, tidy=TRUERUE, dpi=300, out.width='.25\\textwidth'>>=
plotGeneAnnotation(annot.list)
plotGeneAnnotation(annot.list, cluster=TRUERUE)
@
\includegraphics[width=0.3\textwidth]{FiguresplotGeneAnnotation1.pdf}
\includegraphics[width=0.3\textwidth]{FiguresplotGeneAnnotation2.pdf}
...
<<selectBamChipseq, eval=TRUERUE, tidy=TRUERUE>>=
genomationDataPath = system.file('extdata',package='genomationData')
...
<<readCtcfPeaks, eval=TRUERUE, tidy=TRUERUE>>=
...
<<ctcfScoreMatrixList, eval=FALSE, tidy=TRUERUE, fig.width=5, fig.height=3.5, dpi=300>>=
...
@
\includegraphics[width=0.65\textwidth]{FiguresctcfScoreMatrixList.pdf}
...
\begin{figure}
<<plotScaledProfile, eval=FALSE, tidy=TRUERUE, fig.width=5, fig.height=3.5, dpi=300>>=
sml.scaled = scaleScoreMatrixList(sml)
multiHeatMatrix(sml.scaled, xcoords=c(-500, 500), col=c('lightgray', 'blue'))
@
\includegraphics[width=0.65\textwidth]{FiguresplotScaledProfile.pdf}
...
<<readAllPeaks, eval=TRUERUE, tidy=TRUERUE>>=
genomationDataPath = system.file('extdata',package='genomationData')
sampleInfo = read.table(file.path(genomationDataPath, 'SamplesInfo.txt'), 
...
the combinations, put the \Rcode{use.names=TRUERUE}.
<<findFeatureComb, eval=TRUERUE, tidy=TRUERUE>>=
...
<<visualizeFeatureComb, eval=FALSE, tidy=TRUERUE, fig.width=5, fig.height=3, dpi=300>>=
...
multiHeatMatrix(sml.scaled, xcoords=c(-500, 500), col=c('lightgray', 'blue'))
@
\includegraphics[width=0.65\textwidth]{FiguresvisualizeFeatureComb.pdf}
...
% ----------------------------------------------------------------- %
\subsection{Using data from AnnotationHub}
...
<<annotationHubExample1, eval=FALSE, tidy=TRUERUE>>=
